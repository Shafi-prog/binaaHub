...
class IntegratedMedusaClient {
  ...
  async getProducts(params?: { limit?: number; offset?: number }) {
    ...
    const response = await fetch(`${this.baseUrl}/api/medusa/store/products?${searchParams}`, {
  ...
  // Regions API
  async getRegions() {
    const response = await fetch(`${this.baseUrl}/api/medusa/store/regions`, {
      headers: this.headers,
    });
  ...
  async createCart(data: { region_id: string; items?: any[] }) {
    const response = await fetch(`${this.baseUrl}/api/medusa/store/carts`, {
      method: 'POST',
      headers: this.headers,
  ...
  async getCart(cartId: string) {
    const response = await fetch(`${this.baseUrl}/api/medusa/store/carts?id=${cartId}`, {
      headers: this.headers,
    });
