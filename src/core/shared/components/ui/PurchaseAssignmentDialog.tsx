...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/core/shared/components/ui/dialog';
import { Badge } from '@/core/shared/components/ui/badge';
import { Project, Order } from '@/core/shared/types/types';
import { ProjectPurchaseService, PurchaseAllocation } from '@/core/services/projectPurchaseService';
import { 
  Package, 
  Calculator, 
  AlertCircle, 
...
export default function PurchaseAssignmentDialog({ 
  ...
  useEffect(() => {
    if (order && availableProjects.length > 0) {
      ...
      if (assignmentMode === 'single') {
        ...
        const equalQuantity = Math.floor((order.quantity || 1) / availableProjects.length);
        const equalCost = Math.floor((order.totalCost || 0) / availableProjects.length);
  ...
  const handleMultipleAssignment = async () => {
    ...
    if (totalQuantity !== (order.quantity || 1)) {
      alert('مجموع الكميات يجب أن يساوي الكمية الإجمالية');
      return;
    }

    if (Math.abs(totalCost - (order.totalCost || 0)) > 1) {
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{(order.totalCost || 0).toLocaleString('en-US')} ر.س</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">المورد</p>
                  <p className="font-medium">{order.supplier || 'غير محدد'}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className={`text-lg ${getTotalAllocated('quantity') === (order.quantity || 1) ? 'text-green-600' : 'text-red-600'}`}>
                        {getTotalAllocated('quantity')} / {order.quantity || 1}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">مجموع التكاليف المخصصة</p>
                      <p className={`text-lg ${Math.abs(getTotalAllocated('cost') - (order.totalCost || 0)) <= 1 ? 'text-green-600' : 'text-red-600'}`}>
                        {getTotalAllocated('cost').toLocaleString('en-US')} / {(order.totalCost || 0).toLocaleString('en-US')} ر.س
                      </p>
                    </div>
                  </div>
                </div>
