    ...
    for (const expense of data || []) {
      ...
      if (categoryMap.has(categoryId)) {
        ...
        categoryMap.set(categoryId, {
          ...
          category_name: catData?.name ? String(catData.name) : 'أخرى',
          category_name_ar: catData?.name_ar ? String(catData.name_ar) : 'أخرى',
          total_amount: expense.amount,
          transaction_count: 1,
          color: catData?.color ? String(catData.color) : '#999999',
        });
      }
    }
