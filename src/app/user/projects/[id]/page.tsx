...
export default function ProjectDetailPage() {
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      // Create a fresh supabase client instance to ensure auth state
      ...
      console.log('🔍 [fetchProject] Fresh auth check:', { 
        currentUser: currentUser ? { id: currentUser.id, email: currentUser.email } : null,
        authError: authError?.message,
        originalUser: { id: user.id, email: user.email },
        userMatch: currentUser?.id === user.id
      });

      if (authError || !currentUser) {
        console.error('❌ [fetchProject] Authentication failed in fresh check:', authError);
        setProjectError('خطأ في المصادقة - يرجى تسجيل الدخول مرة أخرى');
        router.push('/login');
        return;
      }

      // Use the getProjectById function 
      ...
      if (projectData) {
        ...
        // If getProjectById returns null, let's try a direct query to see what's happening
        console.log('🔍 [fetchProject] getProjectById returned null, trying direct query...');
        
        const { data: directProject, error: directError } = await freshSupabase
          .from('projects')
          .select('id, name, user_id, status, created_at')
          .eq('id', projectId);
          
        console.log('📊 [fetchProject] Direct query result:', { 
          directProject, 
          directError: directError?.message,
          count: directProject?.length 
        });
        
        if (directProject && directProject.length > 0) {
          const project = directProject[0];
          console.log('📋 [fetchProject] Project exists but not accessible:', {
            projectId: project.id,
            projectUserId: project.user_id,
            currentUserId: currentUser.id,
            match: project.user_id === currentUser.id
          });
          
          if (project.user_id !== currentUser.id) {
            setProjectError('ليس لديك صلاحية للوصول إلى هذا المشروع');
          } else {
            setProjectError('خطأ غير متوقع في تحميل المشروع');
          }
        } else {
  ...
  const retryFetchProject = async () => {
    if (retryCount >= 3) {
      console.log('❌ Max retry attempts reached');
      return;
    }

    console.log(`🔄 Retrying project fetch (attempt ${retryCount + 1}/3)...`);
        ...
        <Card className="p-6 text-center">
          ...
          {retryCount < 3 && (
            <button
              ...
              disabled={loading}
              className="mt-4 bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white px-4 py-2 rounded-lg transition-colors"
            >
              {loading ? 'جاري إعادة المحاولة...' : `إعادة المحاولة (${retryCount}/3)`}
            </button>
          )}
          {retryCount >= 3 && (
            <div className="mt-4 p-3 bg-yellow-100 border border-yellow-300 rounded-lg">
              <p className="text-sm text-yellow-800">تم استنفاد محاولات إعادة التحميل. يرجى التحقق من الاتصال بالإنترنت أو المحاولة لاحقاً.</p>
            </div>
          )}
          <button
            onClick={() => router.back()}
