...
export default function ConstructionProductsPage() {
  ...
  const loadData = async () => {
    try {
      // Load categories
      const categoriesResponse = await fetch('/api/categories/construction');
  ...
  const handleBarcodeScanned = async (barcode: string) => {
    try {
      const response = await fetch(`/api/products/construction?barcode=${barcode}`);
      if (response.ok) {
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    try {
      const response = await fetch(`/api/products/construction/${productId}`, {
        method: 'DELETE',
      });
