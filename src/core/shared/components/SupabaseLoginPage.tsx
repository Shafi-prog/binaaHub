...
export default function SupabaseLoginPage() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        console.log('🧪 Testing connection to /api/test-supabase...');
        
        // Use our test API route instead of direct Supabase call to avoid CSP issues
  ...
  useEffect(() => {
    if (user && !isSubmitting && !isLoading) {
      // Ensure we have a valid session and not just loading state
      console.log('User state:', { user, isSubmitting, isLoading, hasSession: !!user.id });
      ...
      if (user.id && user.email) {
        ...
        // Use window.location for a hard redirect that can't be blocked
        setTimeout(() => {
          console.log('🚀 Executing hard redirect...');
          window.location.href = redirectPath;
        }, 500);
      }
    }
  }, [user, router, isSubmitting, isLoading]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-100 flex items-center justify-center p-4" dir="rtl">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
        ...
        {/* Manual Dashboard Access (for debugging) */}
        {user && (
