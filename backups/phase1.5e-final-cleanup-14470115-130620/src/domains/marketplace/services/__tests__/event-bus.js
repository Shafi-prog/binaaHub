...
const event_bus_redis_1 = __importDefault(require("../event-bus-redis"));
...
describe("RedisEventBusService", () => {
    ...
    describe("constructor", () => {
        beforeEach(async () => {
            jest.clearAllMocks();
            eventBus = new event_bus_redis_1.default(moduleDeps, simpleModuleOptions, {
                scope: "internal",
            });
        });
        it("Creates a queue + worker", () => {
            expect(bullmq_1.Queue).toHaveBeenCalledTimes(1);
            expect(bullmq_1.Queue).toHaveBeenCalledWith("events-queue", {
                connection: expect.any(Object),
                prefix: "RedisEventBusService",
            });
            expect(bullmq_1.Worker).toHaveBeenCalledTimes(1);
            expect(bullmq_1.Worker).toHaveBeenCalledWith("events-queue", expect.any(Function), {
                connection: expect.any(Object),
                prefix: "RedisEventBusService",
                autorun: false,
            });
        ...
        it("Throws on isolated module declaration", () => {
            try {
                ...
                expect(error.message).toEqual("At the moment this module can only be used with shared resources");
            }
        });
    });
    describe("emit", () => {
        describe("Successfully emits events", () => {
            beforeEach(async () => {
                ...
                queue = eventBus.queue_;
                queue.addBulk = jest.fn();
                redis = eventBus.eventBusRedisConnection_;
                redis.rpush = jest.fn();
            });
            ...
            it("should add job to queue with module job options", async () => {
                eventBus = new event_bus_redis_1.default(moduleDeps, {
                    ...simpleModuleOptions,
                    jobOptions: {
            ...
            it("should successfully group events", async () => {
                ...
                const [builtEvent] = eventBus.buildEvents([event], options);
                await eventBus.emit(event, options);
                expect(queue.addBulk).toHaveBeenCalledTimes(0);
                expect(redis.rpush).toHaveBeenCalledTimes(1);
                expect(redis.rpush).toHaveBeenCalledWith("staging:test-group-1", JSON.stringify(builtEvent));
            });
            it("should successfully group, release and clear events", async () => {
                ...
                const [testGroup1Event] = eventBus.buildEvents([events[0]], options);
                const [testGroup2Event] = eventBus.buildEvents([events[2]], options);
                const [testGroup2Event2] = eventBus.buildEvents([events[3]], options);
    ...
    describe("worker_", () => {
        describe("Successfully processes the jobs", () => {
            ...
            it("should process a simple event with no options", async () => {
                ...
                expect(loggerMock.info).toHaveBeenCalledWith("Processing eventName which has 1 subscribers");
            ...
            it("should process event with failing subscribers", async () => {
                ...
                expect(loggerMock.info).toHaveBeenCalledTimes(1);
                expect(loggerMock.info).toHaveBeenCalledWith("Processing eventName which has 4 subscribers");
                expect(loggerMock.warn).toHaveBeenCalledTimes(5);
                expect(loggerMock.warn).toHaveBeenNthCalledWith(1, "An error occurred while processing eventName:");
                expect(loggerMock.warn).toHaveBeenNthCalledWith(2, new Error("fail1"));
                expect(loggerMock.warn).toHaveBeenNthCalledWith(3, "An error occurred while processing eventName:");
                expect(loggerMock.warn).toHaveBeenNthCalledWith(4, "fail2");
                expect(loggerMock.warn).toHaveBeenNthCalledWith(5, "One or more subscribers of eventName failed. Retrying is not configured. Use 'attempts' option when emitting events.");
            ...
            it("should retry processing when subcribers fail, if configured - final attempt", async () => {
                ...
                eventBus.subscribe("eventName", async () => await Promise.reject("fail1"), {
                    subscriberId: "2",
                });
                await eventBus
                    .worker_({
                ...
                expect(loggerMock.warn).toHaveBeenCalledWith("An error occurred while processing eventName:");
                expect(loggerMock.warn).toHaveBeenCalledWith("fail1");
                expect(loggerMock.info).toHaveBeenCalledTimes(2);
                expect(loggerMock.info).toHaveBeenCalledWith("Final retry attempt for eventName");
                expect(loggerMock.info).toHaveBeenCalledWith("Retrying eventName which has 2 subscribers (1 of them failed)");
            });
            it("should retry processing when subcribers fail, if configured", async () => {
                ...
                expect(loggerMock.warn).toHaveBeenCalledWith("An error occurred while processing eventName:");
                expect(loggerMock.warn).toHaveBeenCalledWith("fail1");
                expect(loggerMock.warn).toHaveBeenCalledWith("One or more subscribers of eventName failed. Retrying...");
                expect(loggerMock.info).toHaveBeenCalledTimes(1);
                expect(loggerMock.info).toHaveBeenCalledWith("Retrying eventName which has 2 subscribers (1 of them failed)");
            });
        });
