...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'chk_projects_budget_positive' AND table_name = 'projects'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'chk_projects_actual_cost_positive' AND table_name = 'projects'
  ) THEN
    ALTER TABLE projects 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'chk_projects_progress_percentage' AND table_name = 'projects'
  ) THEN
    ALTER TABLE projects 
