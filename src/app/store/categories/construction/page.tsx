...
export default function ConstructionCategoriesPage() {
  ...
  const loadCategories = async () => {
    try {
      const response = await fetch('/api/categories/construction');
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory 
        ? `/api/categories/construction/${editingCategory.id}`
  ...
  const handleDelete = async (categoryId: string) => {
    ...
    try {
      const response = await fetch(`/api/categories/construction/${categoryId}`, {
        method: 'DELETE',
      });
