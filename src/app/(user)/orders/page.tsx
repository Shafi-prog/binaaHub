  ...
  useEffect(() => {
    ...
    const fetchOrdersData = async () => {
      try {
        ...
        console.log('🔍 [User Orders] Verifying authentication...');
        const authResult = await verifyAuthWithRetry(5);

        if (!authResult.user) {
          console.error('❌ [User Orders] Authentication failed');
          router.push('/login');
          return;
        }

        console.log('✅ [User Orders] User authenticated:', authResult.user.email);
        setUser(authResult.user);

        if (authResult.user.id) {
          const ordersResponse = await getRecentOrders(authResult.user.id, 50);
          setOrders(Array.isArray(ordersResponse) ? ordersResponse : []);
        }

        console.log('✅ [User Orders] Data loaded successfully');
      } catch (error) {
        console.error('❌ [User Orders] Error loading data:', error);
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600">إدارة ومتابعة طلباتك من المتاجر</p>
          </div>
