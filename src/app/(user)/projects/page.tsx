  ...
  useEffect(() => {
    ...
    const fetchProjectsData = async () => {
      try {
        ...
        console.log('🔍 [User Projects] Verifying authentication...');

        // Use robust authentication recovery
        const authResult = await verifyAuthWithRetry();
        if (authResult.error || !authResult.user) {
          console.error('❌ [User Projects] Authentication failed');
          setAuthError('المستخدم غير مسجل الدخول');
          router.push('/login');
          return;
        }

        console.log('✅ [User Projects] User authenticated:', authResult.user.email);
        ...
        console.log('✅ [User Projects] Data loaded successfully');
      } catch (error) {
        console.error('❌ [User Projects] Error loading data:', error);
