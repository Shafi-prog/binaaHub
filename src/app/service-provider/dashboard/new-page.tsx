...
import { ServiceProviderDashboardData, ServiceBooking, WarrantyClaim } from '@/core/shared/types/platform-types';
...
export default function ServiceProviderDashboard() {
  const { user } = useAuth();
  const [dashboardData, setDashboardData] = useState<ServiceProviderDashboardData | null>(null);
  const [loading, setLoading] = useState(true);
  const [selectedTab, setSelectedTab] = useState<'bookings' | 'warranties' | 'profile'>('bookings');
  ...
  const handleUpdateBookingStatus = async (bookingId: string, newStatus: ServiceBooking['status']) => {
    try {
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                                            {dashboardData.bookings.filter(b => b.status === 'requested').length}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {dashboardData.bookings
                        ...
                        .reduce((sum, booking) => sum + (booking.serviceDetails.estimatedCost || 0), 0)
                        .toLocaleString()} ر.س
                    </p>
                  </div>
            ...
            <div className="border-b border-gray-200">
              <nav className="flex space-x-8 space-x-reverse px-6">
                {[
                  ...
                  // { key: 'warranties', label: 'الضمانات', icon: Wrench, count: dashboardData?.warrantyClaims?.length || 0 },
            ...
            <div className="p-6">
              ...
              {selectedTab === 'warranties' && (
                <div className="space-y-4">
                  ...
                  {/* Warranty claims feature not implemented yet */}
                  <div className="text-center py-8">
                    <Wrench className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-600">ميزة مطالبات الضمان قيد التطوير</p>
                  </div>
                </div>
              )}
