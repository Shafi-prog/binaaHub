...
const testLoginRedirection = async () => {
    ...
    // Test credentials
    const credentials = [
        { email: 'user@user.com', password: '123456', expectedRedirect: '/user/dashboard', type: 'User' },
        { email: 'store@store.com', password: '123456', expectedRedirect: '/store/dashboard', type: 'Store' }
    ];
        ...
        try {
            ...
            if (result.success) {
                ...
                try {
                    ...
                    console.log(`   Dashboard Access: ${dashboardResponse.status === 200 ? 'Accessible' : `HTTP ${dashboardResponse.status}`}`);
                } catch (dashError) {
